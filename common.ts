...
import { readableStreamToString, FrontChannelError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new FrontChannelError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new FrontChannelError(e, e.response?.data)
                }
                if (innerError instanceof FrontChannelError) {
                    // Got "FrontChannelError" from the above try block
                    throw innerError;
                }
